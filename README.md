...
`dagger -W github.com/ninesl/daggerer@master` loads this module directly from Git. `api call release` invokes the complete release operation described above. The SSH target is the same VPS, using its hostname or network address reachable from the Dagger container. Using SSH here keeps this example identical to a release targeting another server.
...
jobs:
  deploy:
    ...
    env:
      ...
      # Required because this app's go.mod references private GitHub dependencies.
      # Set this GitHub secret to the runner-local filename github_token.
      GITHUB_TOKEN_SECRET: ${{ secrets.GITHUB_TOKEN_SECRET }}
      # Filename of a separate build secret under the same secrets directory.
    ...
    steps:
      ...
      - name: Release
        ...
        run: |
          ...
          # This shell and the Dagger CLI run as the svc.sh service user.
          # $HOME is that user's home directory on the self-hosted runner.
          secrets_path="$HOME/$SECRETS_DIR"
          # Use the Go version declared by the application.
          ...
          # Only public values go in this file; PAT and BAR stay in Secret inputs.
          printf 'GO_VERSION=%s\nAPP_PACKAGE=.\nFOO=hello\n' "$go_version" > "$build_env_file"

          # Check registry authentication first, reuse cached image build work,
          # publish the tag, then use live SSH to pull and deploy it.
          # The SSH target and the self-hosted runner are the same VPS.
          # Defaults: latest, Dockerfile, compose.yml.
          # Container-only secret defaults are also configurable:
          # --ssh-key-mount-path=/run/secrets/ssh_key
          # --known-hosts-mount-path=/run/secrets/known_hosts
          # --registry-password-mount-path=/run/secrets/registry_password
          # prod/$APP_NAME/compose.yml must exist under the SSH user's home.
...
Daggerer supplies `APP_IMAGE=registry/app-name:tag` to the remote Compose process. The existing Compose file must reference that variable for the app service:
